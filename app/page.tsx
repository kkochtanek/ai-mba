...
// Resize + compress image to max 1024px, quality 0.85 before uploading
...
export default function Home() {
  ...
  async function handleVisualize() {
    ...
    try {
      ...
      await Promise.all(rooms.map(async ({ file }) => {
        const compressed = await compressImage(file);
        formData.append("rooms", compressed, file.name);
      }));
      ...
      // Compress + convert room photos to base64 for img2img
      const roomBase64s = await Promise.all(rooms.map(async (r) => {
        const compressed = await compressImage(r.file);
        return fileToBase64(new File([compressed], r.file.name, { type: "image/jpeg" }));
      }));
